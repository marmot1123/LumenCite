...
- **Phase 5（定理・定義・証明）**: `definition`/`theorem`/`lemma`/`proposition`/`corollary`/`remark`/`example`/`proof` ノードを認識する（**新規テーブルなし**・`extractor_version` は pdfium 0.3.0→**0.4.0** / TeX 0.1.0→**0.2.0**）。**TeX** は環境名から種別を決め（preamble の `\newtheorem{env}{Display}` を回収し表示名から独自名・略記を対応づけ、標準英名 + amsthm 予約の `proof` は既定マップ）、`[note]` と `\label` を `payload_json`（`note`/`labels`）へ。本文は 1 ブロックに collapse し内側 display 数式は生 LaTeX のまま残す（別ノード化しない）。**PDF** は行頭キーワード + 番号 + 終端記号で認識し `payload_json` に `{theorem_number, note}`（参照文中の "Theorem 2 shows …" は棄却）。定理間参照グラフ（`proves` 等）は Phase 6（`node_relations`）に委譲。読み出しは汎用経路（`is_content_block` blacklist・node-FTS・`get_document_blocks` の `kinds`）で追加改修なく surface する。
