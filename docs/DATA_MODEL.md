...
v0.2.0 の **LLM Vision OCR**（`ocr_pdf`）はスキャン PDF の認識結果を**この同じテーブル**にページ単位で書き込み、以後 `fulltext_search` でヒットするようにする（スキーマ変更なし）。
...
| `bibtex_sync_path` | 絶対パス | `.bib` 同期先（既存）。⚠ **このキーだけドットではなくアンダースコア**（他の規約に合わないが、既存データがあるので改名しない） |
| `llm.summary_prompt` | 文字列（未設定可） | 要約生成のプロンプト上書き。未設定なら組み込みの既定文 |
...
| `lcir.enabled` | v0.8.0 | `"1"` \| `"0"`（または未設定） | LCIR（機械可読中間形式）の実験フラグ。**v1.0.0-p3 で既定 ON に反転**した ── 判定は「`"0"` でなければ ON」で、**明示的に切った人だけが OFF のまま残る**。⚠ **未設定 =「一度も触っていない」であって「新規ライブラリ」ではない** —— v0.8.0〜v0.10.0 はこれが既定 off の実験フラグで読み手が `== Some("1")` だったため、**その間トグルを触らなかった既存ライブラリは全部未設定**。反転はその全員を ON にする（`lcir.tex_autofetch.enabled` のバックフィルが要る理由そのもの）|
...
| `fulltext.source.<attachment_id>` | `lcir` \| `ocr` | **v1.0.0-p1 の出どころ記録**（添付単位）。FTS5 仮想表に provenance 列を足せないのでここに持つ（migration は v1.0.0 全体で 0 件）。⚠ **「記録が無い」には 3 つの母集団が入る**（この 2 択だと思うと守り方を間違える）: ①この版より前に入った索引（pdf-extract 由来 か **旧 OCR** — 実 DB からも区別できない）②OCR が行を書いてから封印するまでの窓（debt-44）③中断・部分 OCR（**完走時しか封印しない**ので恒常的に生まれる・debt-43）。②③のために `index_attachment_from_lcir` は `protect_unrecorded` 引数を持つ。「記録なし + 索引あり」が旧 OCR か新規 pdf_extract かを区別できないのが debt-37 の窓。⚠ **未知の残骸として掃除しないこと** —— 消すと「記録なし + 索引あり」に戻り、次の自動処理が OCR 由来の索引を LCIR で置き換えうる。逆に添付を消したのに残すと、その `attachment_id` を再利用した添付が永久に索引されない。**後始末は索引・添付・エントリの削除と同一トランザクションで行う**（`db::fulltext::clear_fulltext_source_tx` / `clear_fulltext_sources_for_entry_tx`）|
...
| `lcir.backfill.last_run` | ISO8601 | LCIR バックフィル（v1.0.0-p2）が**最後に 1 件以上着手した**時刻。間引き（`AUTO_INTERVAL_SECS` = 1 時間）の基準。⚠ **「直近実行時刻」ではない** —— 対象 0 件の回・全 skip の回・他バッチに譲って降りた回・LCIR OFF の回は**書かない**（書くと間引きが効いて次の 1 時間ぶんのバックフィルが止まる）。⚠ **「起動時」だけでもない** —— 起動 60 秒後から 10 分周期で「走ってよいか」を叩き続け、実際に走るのは 1 時間ごと（10 分の tick と 1 時間の間引きを混同しないこと。`backfill.rs` のコメントが名指しで戒めている） |
...
- **Phase 8d-7（本文 → 図表参照）**: **新規ノード型・新規テーブルなし**。PDF 本文の "Figure 3" / "Fig. 3" / "Table 2" を図表番号と照合し `node_relations` に `refers_to_figure` / `refers_to_table` を張る（`origin='layout_model'`・confidence 0.6・`metadata_json` に `{ref, number, resolved_via}`）。`resolved_via` は `"node"`（`figure`/`table` ノードに解決）/ `"caption"`（実体が無く caption ノードに解決）。`extractor_version` pdfium 0.6.0→**0.7.0**（TeX は不変）。**同一版で番号が衝突する図表・複数形/範囲参照には辺を張らない**（誤検出より欠損）。既存コーパスは outdated 判定になるが、**再構築は必須ではない**（辺は次回 `rebuild_outdated_lcir` 時に付く）。
...
#### `assets` / `node_assets` — 図表アセット（Phase 8a / migration 0019）

図領域のページ crop PNG 等のバイナリを**ファイルシステムに保存し、DB は相対パス + SHA-256 参照のみ**を持つ（`attachments` と同じ方針・ADR #3）。build 時に `figure` ノードと `role` で紐づける（`node_assets`）。
...
| `relative_path` | TEXT NOT NULL | app data dir 相対・`/` 区切り（下記レイアウト） |
| `width` / `height` | INTEGER | ピクセル寸法 |
| `size_bytes` | INTEGER | ファイルサイズ（容量集計用） |
| `metadata_json` | TEXT | `{page, region_index, render_target_width}` 等 |
| `created_at` | TEXT | `datetime('now')` |

`node_assets`: `id` / `node_id`(FK → document_nodes CASCADE) / `asset_id`(FK → assets CASCADE) / `role` NOT NULL（8a は `page_crop` のみ。将来 `original`/`vector`/`thumbnail`/`ocr_source`/…）/ `created_at`。`UNIQUE (node_id, asset_id, role)`（新規テーブルなので migration 内で張る）。

**ファイルレイアウトとライフサイクル**:

- 保存先は `attachments/<entry_id>/.lcir/<attachment_id>/<content_key 先頭16hex>/fig-p{page:03}-{idx:02}.png`。`attachments/` 配下に置くことで**バックアップ zip・restore allowlist・エントリ purge（ディレクトリごと trash）に無改修で乗る**。バックアップ容量は世代数 × アセット総量ぶん増える（`size_bytes` で集計可能）。
- `relative_path` は OS パス演算でなく `/` の文字列連結で構築する（Windows で `\` を DB に入れないため。`attachments.file_path` と同じ規約）。
