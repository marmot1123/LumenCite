...
論文全文を「型付きノード木 + PDF 座標 + provenance + 信頼度」で保存する中間表現 **LCIR**（LumenCite Document Intermediate Representation）の基盤。設計全体は `docs/LCIR_design_overview.md`。**実験段階**で、settings `lcir.enabled` が `"1"` のときだけ pdfium 抽出で構築する追加の side-build（既存 `fulltext` は不変）。第一段は下記 3 表のみ（math/assets/relations/symbols は後続フェーズの別 migration）。
...
| `content_key` | TEXT NOT NULL | `sha256(source_sha256\|extractor_name\|extractor_version\|config_hash)`。**再現可能な内容由来 ID**（同一 PDF+同一抽出器 → 同一値）。row id は採番で再現不能なため。best-effort UNIQUE は `(attachment_id, content_key)`（起動時 `try_create_content_key_unique_index`・重複 DB で brick させないため migration では張らない） |
...
| `extractor_name` / `extractor_version` | TEXT NOT NULL | 例 `lumencite-pdfium` / `0.1.0`。version は**抽出ロジックの semver**（supersede 判定基準・pdfium クレート版とは別） |
...
座標は既存 `highlights` と同一系（PDF user space・左下原点・pt）。1 ノードが複数ページ/領域にまたがれば複数行。各 page ノードには常にページ全面（MediaBox）の fragment を 1 つ付与する。
...
段落・見出し・caption 等の**ブロック粒度**の全文検索用 FTS5 仮想表（trigram）。ページ粒度の既存 `fulltext` と併存する派生索引で、正本は `document_nodes`。LCIR build 時に `ingestion::regenerate_node_fts_from_lcir` が張り、`document`/`page`/`line` を除く本文つきブロックだけを載せる。
...
FTS5 仮想表なので attachments への FK は張れず、hard delete 時は `db/entries.rs` の削除経路が `attachment_id` 指定で手動クリーンアップする（`fulltext` と同型）。

#### `math_expressions` — 数式の複数表現（Phase 3 / migration 0016）

`inline_math`/`display_math` ノードに 1:1 で付く。数式は単一形式に統一せず用途別の表現を列で併存させる。**PDF 由来はまず表層のみ**（`normalized_text` = 正規化した Unicode 線形文字列）を埋め `semantic_status='surface_only'`。LaTeX は Phase 4（TeX 取込）、Content MathML/OpenMath/AST は Phase 7（意味）で埋める。ノード削除でカスケード消去。
...
| `latex` / `presentation_mathml` / `content_mathml` / `openmath_json` / `ast_json` | TEXT | 後続フェーズで埋める（PDF 由来は NULL） |
