...
| コード側の抽出器版 | pdfium **`0.11.0`**（#3 で 0.10.0 から bump）/ tex `0.5.0` → **PDF 138/138 が outdated（実 DB は全件 0.6.0）・TeX は 0 件** |
...
| disk | 89 GB 空き（91% 使用） |
| テスト本数 | **1,019**（#3 時点の `cargo test --lib` 実測。#0 で 977・#1 で +8・#2 で +12・#2.5 で +7・#3 で +15） |
