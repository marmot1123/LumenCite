...
| 8 | ~~`feat(LCIR): 全文検索の索引源を LCIR 優先にする（p1）`~~ **完了**（§2.18） | p1 | M / 900–1,250 → **実績 1,990 追加 / 214 削除**（テスト込み） | なし | 否 | 再構築の**後**。p1 の再導出は pdfium 不要の純 SQL（秒オーダー）なので再構築に相乗りさせる利得がほぼ無い一方、debt-17 / debt-22 の解消を含むので独立してレビューしたい。OCR 保護を settings に置く決定（§2.6-1）により **migration は不要**になり、配布版 v0.10.0 との併用も壊れない |
...
| p1 の派生化（**2026-08-07・#8・実 DB のコピーで実測**） | `fulltext` **5,710 → 5,822 行**（+112 / **−0**）・**C0 を含む行 729 → 0**（受け入れ条件）・索引を持つ添付 **133 → 135**（att93 +15p / att94 +42p）・所要 **12〜20 秒 / 138 添付**（純 SQL）。バッチ内訳は対象 138 / 派生 134 / `skipped_empty` 4（スキャン本）/ `skipped_ocr` 0 / `failed` 0。**同じページの内容は 5,710 中 5,666 で置き換わる**（長さまで同じは 44 のみ）＝「重複しているだけ」ではない。頻出語 12 語は**全語で純増**だが、内訳は **既存ページで新たに引ける 512 / 新規ページ 298 / 引けなくなった 216**（置き換えだけ見ても 512 対 216 = 2.4 倍）。**「減少 0」ではない** ── 語ごとの総数しか見ていなかった初回計測ではこの 216 件が見えていなかった |
| disk | 89 GB 空き（91% 使用） |
| テスト本数 | **1,142**（#8 時点の `cargo test --lib` 実測・keychain の 1 本を skip した数）。#0 で 977・#1 で +8・#2 で +12・#2.5 で +7・#3 で +15・#4 で +32・#5 で +27・#6 で +39・**#8 で +25**（うち 6 本は変異で、6 本はコミット後レビューで穴が見えてから追加。`#[ignore]` の実 DB プローブ 1 本は別） |
