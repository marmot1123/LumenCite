...
- **所要 71.05 秒**（索引あり・チャンク込み）。最遅の版は 7.95 秒だが、それは
  272,583 ノードの版が **6 チャンク**に割れたときの合計で、**1 tx あたりは約 1.3 秒**。
...
テスト 1,078 → **1,107**（+29・`#[ignore]` のプローブ 1 本を含めて +30）。
...
| p4 の GC（実 DB のコピーに本番の `run_gc` を流した実測・2026-08-05） | superseded **145 → 0** / `document_nodes` **2,663,234 → 450,149** / `source_fragments` 2,659,223 → 448,241 / `node_alt_texts` **888 のまま** / 再利用可 **0 → 493,883,392 B（471.0 MiB）** / ファイルは縮まない。**所要 71.05 秒**（`symbols(scope_node_id)` 索引あり + 50,000 ノードのチャンク）。最遅の版は 7.95 秒だがこれは 272,583 ノードが 6 チャンクに割れた合計で **1 tx は約 1.3 秒**。索引なし・チャンクなしだと **125 秒 / 版あたり最大 16.7 秒**（5 秒超が 6 版） |
...
| disk | 89 GB 空き（91% 使用） |
| テスト本数 | **1,107**（#6 時点の `cargo test --lib` 実測・keychain の 1 本を skip した数）。#0 で 977・#1 で +8・#2 で +12・#2.5 で +7・#3 で +15・#4 で +32・#5 で +27・**#6 で +29**。`--list` の総数は 1,123（`#[ignore]` 13 本を含む） |
