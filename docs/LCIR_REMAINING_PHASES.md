...
| 区分 | 項目 |
|------|------|
| **v1.0.0 に載せる（8 項目）** | `p1` FTS 派生化 / `p2` 自動 build / `p3` 既定 ON + 同意分離 / `p4` superseded GC + 容量可視化 |
| | 図の実害の一括解消: `8d-2` ベクター図 / `8d-8` XObjectForm・回転頁 / `debt-14` CropBox クランプ |
| | `debt-12` caption 取りこぼし |
...
| 5 | （PR なし）**ゲート ②a — 抽出器差分だけの限定レビュー** | 1〜4 の diff | — | なし | 否 | ここで出た指摘は版 bump のみで吸収でき再構築を増やさない。#7 の後に回すと、抽出器への指摘 1 件で 80 分再構築 + carry 再賭け + Vision 再課金がもう 1 回確定する |
...
| 7 | （PR なし）**唯一の再構築 80 分 + Vision 1 回** | — | — | なし | **実行** | 版を 4 回上げても実データは動いていない。ここで人が 1 回押す |
...
pdfium の版は `0.7.0 → 0.8.0 → 0.9.0 → 0.10.0 → 0.11.0` の 4 回上がる（TeX 側は不変）。
...
debt-19（テキスト fragment 11,963 件がページ矩形をはみ出す）と回転ページのせいで RFC 8118 の `viewrect` を
...
2. **その原点の求め方**（`figures::effective_page_box_origin`・`page_box_origin` から呼ぶ）。
   生 CropBox（無ければ MediaBox）→ **`CropBox ∩ MediaBox`**。pdfium のページ寸法は交差の寸法なので、
   原点だけ生 CropBox から取ると寸法と原点の出所が食い違う。**クランプだけ直して原点をそのままにすると
   「ページ box 原点基準にする」が半分しか成立しない**ので同じ PR に入れた。

   実測（生存 138 版 7,345 頁・同梱 libpdfium に直接問い合わせ）: 交差モデルは box を持つ
   **7,281 頁すべて**で `FPDF_GetPageWidthF` を再現し、生 CropBox モデルは **406 頁で外れる**。
   両モデルで原点が食い違うのは **392 頁 / 2 版**（vid 149 の 382 頁・vid 223 の 10 頁。どちらも
   CropBox が (0,0) 始まりで MediaBox の原点が非ゼロという形で、ずれ幅 78–88pt）。交差が空になる頁は 0。
...
| att77 p9 | **bbox 変化**（幅 314.06 → 356.40） | 同上 |
...
**新規 4 件のうち 3 件は論文の図ではない**（誌名バナー・表紙・小さな印）。これは今回の変更が作った
...
**テストは 11 本**（`ingestion::figures::tests` の `clamp_*` 8 本 + `box_origin_*` 3 本）。
**5 通りの変異（4 辺の原点を個別に落とす + 面積ガードの無効化）がすべて狙ったテストだけを落とすことを
確認済**。特に「左端・下端を跨ぐ矩形」のテストが無いと `x0 = rect.x.max(box_left)` の変異が
1 本も落とさずに通る（実際に一度素通りさせてから足した）。

**残す道具**: `figure_regions_real_pdf` は #3（8d-8）/ #4（8d-2）でもそのまま使える。
矩形を足す変更を入れたら、まず 138 本に当てて「増えた図」と「動いた図」を分けて数えること
（1,202 という基準値がここにある）。
...
| **debt-18** | 同じ座標系の取り違えが `structure.rs:365-368` の `in_margin` 判定にもある（`bbox.y` は絶対・`page_height` は box の高さ）。非ゼロ原点の PDF で判定帯がずれ、短い散文行が paragraph → unknown_block に降格する。**2026-08-02 に #2 の調査で測り直した（旧記述「vid 183 で 110 件 vs 51 件」は誤り）**: 誤った帯 / 正しい帯 = vid **149: 1,179 / 272**・183: 100 / 107・202: 24 / 13・223: 22 / 13（1 行 + 8 語以下のゲート込み）。実害の本体は vid 149 の**約 900 件の誤降格**で、doc は過小かつ別の版を挙げていた。**修正は 2 行 + テストで、#7 の唯一の再構築に相乗りできる**（post-1.0 に回すと同じ効果にもう 1 回 80 分かかる） | S | debt-14 と同根・別 PR |
...
- **`CHANGELOG.md`** — Keep a Changelog / SemVer 準拠で実際に維持されており、`## [Unreleased]` に
  Phase 10a/10b が既に積まれている。中間スコープ 8 項目ぶんの整理が要る
...
| debt-18「vid 183 で実測 110 件 vs 正しい帯 51 件」（同上） | **版も件数も誤り**。実測は vid **149 で 1,179 件 vs 272 件**（vid 183 は 100 vs 107 で、負の原点なので誤った帯の方が**少ない**）。実害の本体は vid 149 の約 900 件の誤降格 |
