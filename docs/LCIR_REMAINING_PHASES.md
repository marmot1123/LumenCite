...
恒等行列の form の子 3 枚（2 解釈が同一なので当然）と、**非恒等 form の子 8 枚**（vid263 p14 の
342×455pt の大きな form。子の生矩形が偶然その内側に落ちる）。子 1 枚ずつ判定していたら
この 8 枚は同率のまま決め手が無いが、**同じ form の他の子は生矩形だと form の外**（包含率 0.0）なので、
面積で重み付けした form 全体の比は `FormLocal` 側が厳密に勝つ。
...
予算も分けた（`MAX_FORM_IMAGES_PER_PAGE` = 256 は top-level の `MAX_RAW_RECTS_PER_PAGE` とは別カウント）
── 合算にすると「250 枚 + form 5 枚」のページが新たに丸ごと skip され、**今出ている図が消える**。
...
（debt-25 の隣）であり、8d-2 で図領域の母集団が固まってから 1 回で決めるべきである。
...
**純関数のテストを 13 本**追加（`ingestion::figures::tests` の `transform_bbox_*` 2 /
`compose_affine_*` 2 / `containment_ratio_*` 1 / `calibration_*` 8）。
座標は実データ（att134 p1 の form 行列 `0.4489` 倍 + `(317.014, 450.913)` 平行移動と、
その form の bounds）をそのまま使っている。

**Red は目で確認した**（4 関数の本体を潰すと 13 本中 12 本が assertion で落ちる。
残る 1 本 `calibration_picks_page_space_...` は潰した戻り値と偶然一致するので、
その 1 本だけに頼らない構成にしてある）。**変異 10 通りがすべて狙ったテストだけを落とすことを確認済**:
...
| top-level に Image はあるが図にならない（フィルタで落ちる） | **21** | **103** | どちらでもない**別の穴** |
...
| disk | 89 GB 空き（91% 使用） |
| テスト本数 | **1,017**（#3 時点の `cargo test --lib` 実測。#0 で 977・#1 で +8・#2 で +12・#2.5 で +7・#3 で +13） |
