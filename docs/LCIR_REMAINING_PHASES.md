...
| バックアップ | フル zip（`VACUUM INTO` + attachments 全体・差分も dedup も無し）・keep=14・24h 間隔。現在 5 本 × 831,617,064 B = 3.9 GiB。db.sqlite は raw 737MB → 圧縮 220.6MB。**1 回の所要は 8〜37 分**（2026-08-06 実測 8.5 分 / 過去 5 世代は 8・25・33・8・37 分）で、生 1.5 GiB の deflate が本体（`VACUUM INTO` は数秒）。**進捗表示が無い**（debt-31） |
