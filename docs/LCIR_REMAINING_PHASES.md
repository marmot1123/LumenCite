...
| **10a** | 文脈バンドル（`get_node_context`） | **M** | 6a/6b/8a/8c（済） | 不要 | **実装済**（2026-07-29） |
| **10b** | チャットへの LCIR 露出 + provenance 付き回答 | **M〜L** | 10a | 不要 | **実装済**（2026-07-29） |
...
**エージェント表面の現状（10b 実装後）**: MCP サーバー **17** ツール / アプリ内チャット **最大 20** ツール
（既存 11 + 文献本文 9。ただし LCIR 8 種は「読める版が実在する」ときだけ一覧に出る）/ CLI **14** サブコマンド。
文献本文の read ツールは `llm::tools::document` が正本で、**MCP とチャットが同じ定義・同じ実行を共有する**。
issue #42（索引済み PDF を再 OCR する穴）は `get_fulltext` の露出と `ocr_pdf` 側のガードで解消済み。

| 分割 | 内容 | 難易度 | 概算行数 | migration |
|------|------|--------|----------|-----------|
| 10a | 文脈バンドル（定理 + 前提定義 + 証明 + 参照数式を server-side 結合）| M | 500–700 | **不要** — **実装済**（2026-07-29） |
| 10b | チャットへの LCIR ツール露出 + provenance 付き回答 | M（バックエンド）/ L（bbox ハイライト UI 込み） | 900–1,400（+UI 400–600） | 不要 — **実装済**（2026-07-29） |
| 10c | embedding / ベクトル検索 / 文献横断グラフ | XL | 2,500+ | 2 表以上 |

**10a は実装済（2026-07-29）。** 設計と実装の詳細は `LCIR_design_overview.md` の「Phase 10a 実装状況」。
要点だけ再掲する:

- 中身は `context::build_node_context`（`LcirDocument` → `NodeContext` の**決定的純関数**・DB/fs に触らない）。
  DB 入口は `ingestion::load_node_lcir`。migration なし・新表なし・新しい永続推定なし。
- **引数は `node_id` のみ**（`entry_id` も `source` も取らない）。ノード id が版を決めるので、
  エントリ起点の read 優先度（tex > pdfium）で選び直すと呼び出し側の id が引けない版に化ける。
- **中核は読み順スパン（`continuation`）**。PDF 版の定理ノードは主張の先頭 1 ブロックしか持たず
  （実測 平均 168 字）、続きは page 直下の兄弟に落ち、theorem の 33% / proof の 53% がページをまたぐ。
  木の pre-order で 1 本の列にして次の構造境界の手前まで連結することで完了条件 (2) を満たす。
- **前提定義は辺だけでは 1.4% しか埋まらない**ので、`reference` / `occurrence` / `symbol` の
  3 経路を `via` で区別したまま返す（`symbol` のみ読み側の表層照合・非永続）。
- **2 ホップしない**（`proofs` の node_id で呼び直す）。届かなかったものは `notes` の
  機械可読コードで返す（`export::warning` とは別チャネル・「狼少年にしない 3 規約」は共有）。
- 露出は MCP `get_node_context` + CLI `node-context`。テストは `context` 純関数 30 本 +
  `mcp_server` 6 本 + `cli` 2 本（すべて `#[sqlx::test]` / `#[test]` で CI 完結）。

**debt-8 の警告チャネルを MCP read 面にも出すか**（10a 着手時に再検討すると保留していた件）は
**出さない**で決着した。`export::ExportWarning` は `FormatCapabilities`（= 出力形式の表現力）から
導く「この形式では運べない」であって、10a が言いたい「このバンドルが届かなかった」とは別事実。
型を共有すると `FormatCapabilities` が形式でないものを表すことになる。10a は自前の
`ContextNoteCode` を持ち、規約（実際に起きたことだけ / 常に真の一般論は載せない / 1 事実 1 コード）
だけを共有する。

**10b は実装済（2026-07-29）。** 詳細は `LCIR_design_overview.md` の「Phase 10b 実装状況」。
残った限界だけ再掲する:

- **根拠チップは PDF 由来 LCIR 限定**。TeX 版には `source_fragments` が無く座標を持たないので、
  `load_entry_lcir` が tex を優先する arXiv 論文では既定で根拠チップが出ない。
  ロードマップ完了条件 (1)「LLM 回答から根拠ノードと PDF 領域へ移動できる」は
  **PDF 由来 LCIR について**満たす。tex → pdf の位置解決（定理番号・数式番号での照合）は post-1.0。
- **完了条件 (3)「数式を構造化表現として渡せる」の読み**: TeX 由来の `latex`（原文文字列）を
  もって満たしたと見なす。PDF 由来は `surface_only` で LaTeX を持たず、MathML / Content MathML は
  Phase 7（post-1.0）。v1.0.0 の「Phase 10 到達」を判定するときはこの読みを前提にする。
- **一覧のゲートは spec 面のみ**。`lcir_readable` が false でも `execute_tool` は名前で実行できる
  （過去ターンの履歴に残った呼び出しが再送されても壊さないため）。読み取り専用なので意図的に許容する。
- **プロンプトキャッシュが無い**ので、ツール定義ブロック（20 ツールで約 7k トークン）は毎ターン
  再送される。`cache_control` の導入は p3 と束ねる別 PR。
- **`DEFAULT_MAX_TURNS = 12`** のまま。多ホップ読解では上限に当たりうるが、当たっても
  `Ok(())` で静かに終わる（通知が無い）。上限引き上げ + 終了通知は p1/p2 と束ねる。
...
| debt-14 | **図の bbox が CropBox 原点のずれた PDF で右/上に切れる**。`ingestion/pdf/mod.rs` の クランプが絶対座標 `x`/`y` を CropBox の**幅・高さ**（右端・上端ではない）と比べており、`page_box_origin` の減算は crop 描画側だけに入っている。CropBox が `[20 30 632 822]` の雑誌 PDF では全幅の図が右に 20pt 短く保存され、8a の crop PNG も同じ分を失う。検出自体は通るので**無言**。段落の bbox はクランプしないので影響しない（Phase 10b の批評で発見・pre-existing）。直すと図の再抽出が要るので 8d-2 の再構築と束ねる | S（修正）/ L（再構築込み） | 8d-2 |
...
3. ~~**10a（M）**~~ → ~~**10b（M）**~~ — **どちらも 2026-07-29 実装済**。v1.0.0 の「Phase 10 到達」はここ
...
**v1.0.0 までの最短経路** = ~~p0 → 安い債務 3 件 → 10a → 10b~~（済） → **p1/p2/p3**。いずれも M 以下。
