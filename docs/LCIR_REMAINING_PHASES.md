...
1. **`gc_stale_asset_dirs` に mtime 猶予**（`ingestion/mod.rs`・debt-15）。
   ...
   判定は `is_stale_asset_dir` に切り出し、**ディレクトリ自身ではなく中のファイルの mtime**の
   最大値を見る。`heal_missing_assets` は同じファイル名へ上書きするので、Unix ではディレクトリの
   mtime が動かず「古い」と誤判定されるため。ファイルが 1 つも無いときだけディレクトリ自身の
   mtime にフォールバックする（書き始める直前の別インスタンスを守る）。mtime が読めない /
   未来のときは**回収しない**（残せば次回 build で回収されるだけ・消し違えると別インスタンスの
   成果物が消える、と非対称なので疑わしきは残す）。
...
sweep は削除操作のたびと次回起動時に再試行されるので取りこぼしは残らない。
**p2 の読み手**（バックフィルを走らせてよいインスタンスか）は同じ関数をそのまま使える。
