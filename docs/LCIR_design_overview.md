...
| 8 図表機械可読化 | `assets`/`node_assets`・図切出/SVG/OCR・表セル・plot | F | 1,2 | XL | **8a 実装済**（assets 基盤 + 図 crop + figure ノード + caption_of）。**8b 実装済**（TeX tabular セル構造化・`table` ノード）。8c（alt text）予定 |
