...
| v1.0.0 #11.5 ゲート ②c（Codex）裁定 PR C-01〜C-07 — 同一ライブラリの single-instance 化 / restore 順序 / 同意の直前再評価 / クリッパー通知 / TeX 上書きの原子化 / LLM 設定の 1 tx 保存 | #（後埋め） | （squash 後埋め） | 2026-08-15 | — | 不変（`0.14.0`・抽出出力は変わらない） |
