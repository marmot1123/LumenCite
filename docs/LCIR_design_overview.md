...
| v1.0.0 #3 8d-8 XObjectForm 内の画像 | #78 | `6386ef5` | 2026-08-03 | — | → `0.11.0` |
| v1.0.0 #4 8d-2 ベクター図の領域検出 | #80 | `95a9f65` | 2026-08-03 | — | → `0.12.0` |
| v1.0.0 #5 PR-a ゲート ②a の指摘反映（form 子 path のクリップ交差 + アダプタ層の配線テスト） | #82 | `7b9857a` | 2026-08-04 | — | → `0.13.0` |
...
| v1.0.0 #8 p1 全文検索の索引源を LCIR 優先に / debt-17 / debt-34 | #88 | `88d1fc0` | 2026-08-07 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| v1.0.0 #9 p2 LCIR の自動 build（添付 3 経路 + 起動時バックフィル）/ debt-24 バックエンド側 | #90 | `b69863d` | 2026-08-07 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| v1.0.0 #10 の前提 p0 Linux 実バンドルの pdfium 検証を CI に恒久化（静的 + 実行時） | #92 | `d34452a` | 2026-08-07 | — | 不変（コード変更は探索候補のコメントのみ） |
| v1.0.0 #10 の前提 debt-32 長時間バッチの状態をバックエンド正本に（`lcir_batch_status`） | #94 | `81dc2d6` | 2026-08-07 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| **v1.0.0 #10 p3 `lcir.enabled` 既定 ON + e-print 自動取得の同意分離（`lcir.tex_autofetch.enabled`）** | **#95** | `0f7893b` | 2026-08-07 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| v1.0.0 #11 ゲート ②b PR-1+PR-1b 同意ゲートの一貫化 + 止められる OCR | #99 | `d5533df` | 2026-08-12 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| v1.0.0 #11 ゲート ②b PR-2 索引の保全（空抽出で既存索引を消さない / 再導出の判定を tx 内へ） | #100 | `3472680` | 2026-08-12 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| v1.0.0 #11 ゲート ②b PR-2b バックアップと crop 削除のすれ違い（`SKIPPED.txt` / GC の一時停止） | #103 | `8822395` | 2026-08-13 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| v1.0.0 #11 ゲート ②b PR-3 課金バッチの排他と行き先（3 入口の相互排他 / 版 + crop 指紋の条件付き INSERT） | #104 | `9796c9c` | 2026-08-13 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| v1.0.0 #11 ゲート ②b PR-4 doc 一括点検 + 残り指摘を債務へ（debt-47〜58）／詳細パネルの打ち切り理由 | #105 | `d7f2f7c` | 2026-08-13 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| v1.0.0 #11.5 ゲート ②c（Codex）裁定 PR C-01〜C-07 — 同一ライブラリの single-instance 化 / restore 順序 / 同意の直前再評価 / クリッパー通知 / TeX 上書きの原子化 / LLM 設定の 1 tx 保存 | #107 | `51ca52c` | 2026-08-16 | — | 不変（`0.14.0`・抽出出力は変わらない） |
| **v1.0.0 #12 リリース** — 版 bump 0.10.0 → 1.0.0 + CHANGELOG 切り出し + タグ前検証の反映（CHANGELOG の網羅漏れ 4 件 / `RELEASE.md` の作業事故 2 件 / 債務表の分割 / SPEC に非目標を新設） | #109 | `1853b55` | 2026-08-17 | — | 不変（`0.14.0`） |
...
図 caption ペア 262 → **662**・`refers_to_figure` 99 → **1,209**・page の C0 制御文字 5,786 頁 → **0**・
alt text 888 → **1,234** になった。所要は再構築 20 分 + Vision 30 分（≒$0.80）。

**migration は 0014〜0020 の 7 本**（すべて新規テーブルの追加で、既存表の破壊的変更なし）。
v1.0.0 の残スコープに migration は無い。

**索引だけは migration の外で 2 本張っている**（起動時 best-effort・`_sqlx_migrations` を動かさないので
旧バイナリと DB を共有していても壊さない）: `document_versions(attachment_id, content_key)` の UNIQUE
（重複があると張らない）と、**`symbols(scope_node_id)`**（v1.0.0 #6 で追加。migration 0018 が張り忘れた
唯一の子キー索引で、無いと版の削除がノード 1 件ごとに `symbols` を全走査する ──
実 DB の GC で 125 秒 → 48 秒）。

**出荷**: Phase 0-4 = v0.8.0（2026-07-22）／Phase 5・6 = v0.9.0（2026-07-23）／
Phase 9a・8a/8b/8c・再構築 UI = v0.10.0（2026-07-28）。**ここまでは `lcir.*` が全て既定 OFF**。
**Phase 10a/10b・8d-2/8d-7/8d-8・p1〜p4 = v1.0.0（2026-08-17）で、ここで `lcir.enabled` が既定 ON に反転した**
── 既定 ON 化 + Phase 10 到達が v1.0.0 の看板で、**その看板は出荷された**。
v1.0.0 は **pdfium を Windows / Linux にも初同梱**した版でもある（両 OS で LCIR と OCR が動くのはこの版から）。
...
**⚠ この 6 項目は第一段（Phase 0〜2）の計画で、1 と 3〜6 は v1.0.0 で追い越された。**
現在地を各項目に併記する（初版の記述を消さないのは、なぜその形を選んだかが §8 の
「並走(A) → 派生化(B)」の判断の前提になっているため）。

1. 既存添付ごとに `document_versions` を作成。~~lazy: `build_missing_lcir` コマンドで明示実行。起動時 pdfium 一括掃引はしない~~
   → **v1.0.0-p2 で起動時バックフィルを入れた**（`ingestion::backfill`）。`lcir.enabled` は
   v1.0.0-p3 で**既定 ON** なので、明示的に切っていないユーザーではリリースビルドの起動 60 秒後から
   pdfium 抽出が走りうる。⚠ **周期は 2 段ある** ── `POLL_INTERVAL`（10 分）は「走ってよいか」を
   叩きに行くだけの tick で、実際に走るかは `lcir.backfill.last_run` を見た
   `AUTO_INTERVAL_SECS`（**1 時間**）の間引きが決める。**負荷の頻度は最大 1 時間に 1 回**で、
   間引かれる回は SELECT 2 本で降りる（この 2 つを混同すると負荷を 6 倍に見積もる）。「一括掃引はしない」の趣旨（起動を巻き添えにしない）は
   **1 ラン 5 分の時間予算・添付境界での譲り合い・他バッチとバックアップ中は stand down・
   dev 既定オフ**という形で維持している（かつての「別インスタンス起動中は走らない」ゲートは、
   ②c C-01 で同一ライブラリの第2インスタンスが起動自体を拒否するようになったため削除された）。
   「LCIR は明示実行しかしない」を**切り分けの前提に使わないこと**（起動数分後の CPU / ディスクの
   跳ねはここが原因でありうる）。
2. pdfium 再抽出で `page` / `text_block` ノードを生成（既存 `fulltext` は座標を持たないため、そのまま LCIR 化はできず**再抽出可能な PDF のみ**座標付き LCIR を得る）。
3. ~~既存 `fulltext` は legacy index として維持~~ → **v1.0.0-p1（#8）で派生化(B)へ移行済み**（§8）。
   legacy として並走させるのではなく、`fulltext` そのものを LCIR から張り直す形になった。
4. ~~LCIR 由来の新インデックスを並行運用し検索品質を比較~~ → **実施済み**（#8 の実測。頻出語 12 語で
   全語純増・既存ページで新たに引ける 512 対 引けなくなった 216・§2.18）。並行運用の期間は置かず、
   **実 DB のコピーに本番の派生バッチをそのまま流し、本番の `search_fulltext` で前後を突き合わせた**
   （`p1_derivation_on_a_copy_of_the_real_library`）。⚠ §8 末尾の「比較自体は実 DB の**読み取り専用
   SQL だけ**でできる」は 2026-07-30 の**見込み**で、実施はこの破壊的な試行の方。
   再検証を「SELECT を流すだけ」と見積もらないこと。
5. ~~十分な互換性が確認できたら新インデックスを既定化~~ → **完了**（v1.0.0-p1 + p3 の既定 ON）。
6. ~~legacy index は再生成可能になった時点で削除候補~~ → **不要になった**（3 の派生化で `fulltext` が
   LCIR の派生物そのものになったため、削除すべき別表が存在しない）。OCR 由来の行だけは
   再生成不能なので `settings.fulltext.source.<attachment_id> = ocr` で不可侵にしてある。
