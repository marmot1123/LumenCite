...
| 6 記号/参照グラフ | `symbols`/`symbol_occurrences`・`node_relations`・スコープ | E | 2,3 | L | **一部実装済**（**6a 参照グラフ = `node_relations`・実装済**／6b 記号系 = 予定） |
...
**推奨実装順序**（ロードマップ §11 を LumenCite に合わせて）: 1 → 2 → 3(表層) → 4 → **取得整備（クリッパー欠落補完・TeX 一括取得バッチ — Phase 5 が TeX の恩恵を最も受けるため先に取得面を固める・SPEC.md 参照）** → 5 → 6 → 8 → 7(意味) → 9 → 10。Content MathML・OpenMath・図の意味解析は重要だが**最初から完全実装を目指さない**。まず原資料・位置・構造・由来を失わない基盤（Phase 1）を作る。

**リリースとの対応（2026-07-19 決定・詳細は SPEC.md「v0.8.0 > リリース方針」）**: v0.8.0 = 取得整備と同時（Phase 5 前）。以後はフラグ付きで main に積み、リリースは 2〜3 フェーズごとに間引く。**Phase 9/10 到達 + `lcir.enabled` 既定 ON 化 = v1.0.0 の看板**。
...
| `symbols` | 記号定義（surface_form/normalized_form/description/symbol_type/scope/semantic_json） | 6b | 0018+ |
| `symbol_occurrences` | 数式・本文中の記号出現 → 定義への関連付け | 6b | 0018+ |
