...
**重複クリップ時の欠落補完（設計済み・未実装・2026-07-19）:**

現状、重複クリップ（エントリが既に在る）は副作用ゼロ — エントリに PDF/TeX が欠けていても補完しない。これを「欠落分だけ補完する。ただし**初回は確認**を取り、以後は確認なしを選べる」に拡張する設計。**確認 UI はツールバーボタン直下の拡張ポップアップ**（ユーザー要望・2026-07-19）— クリックした場所で確認が完結し、ブラウザ→アプリをまたぐ非同期 UX を持ち込まない。
...
- **新エンドポイント `POST /clipper/complete`**: `{entry_id, remember?: bool}`（同一 Bearer 認証・`clipper.enabled` ゲート）。アプリ側で欠落を**再検証**してから既存の `spawn_pdf_job` / `spawn_tex_source_job` を発行し、`remember` なら `clipper.complete_missing="1"` を保存。PDF URL・arxiv_id はクリップ時の値をアプリ側で保持せず、エントリの識別子から再導出する（arXiv 導出 PDF / e-print。`citation_pdf_url` 由来の補完はこの版では対象外 = arXiv 前提で十分）。
- **AddSheet 側も同じ設定に従う**: `create_entry` の CR-019 dedup で既存エントリが返るケースはアプリ内の同期モーダルで確認（選択肢は同じ 3 つ）。あわせて既知の quirk を修正する — 現状 AddSheet はチェック ON だと**既存エントリへ PDF を重複添付し得る**（`download_arxiv_pdf` を無条件実行）ため、「PDF 欠落時のみ実行」に変える。
- **実装見送りの理由（2026-07-19）**: 補完実行部（アプリ側）は既存ジョブの再利用で軽いが、**拡張の改修**（ポップアップページ新設・`openPopup` の互換フォールバック・zip 再配布 = ユーザーの手動更新）と実ブラウザでの E2E 確認を伴うため、拡張を更新するセッションでまとめて行う。

**arXiv TeX ソースの一括取得バッチ（設計済み・未実装・2026-07-19）:**
...
**実装タイミング（取得整備セッション・2026-07-19 決定）**: 上記 2 件（重複クリップの欠落補完 + 一括取得バッチ）と AddSheet の PDF 重複添付 quirk 修正を、**次の実装セッション**で「取得整備」としてまとめて行う（LCIR Phase 5 の前 — Phase 5（定理/証明）は TeX ソースの恩恵を最も受けるため、取得面を先に固める）。拡張 zip の再配布を伴うため、リリースは拡張更新と同期させる。
