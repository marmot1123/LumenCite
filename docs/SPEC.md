...
文献メタデータ取得の精度向上と、漢字圏・キリル圏の著者を一級市民として扱うため `authors` テーブルを大幅拡張する。スキーマ定義の詳細は `docs/DATA_MODEL.md` の `authors` / `author_identifiers` セクション、確定経緯は `memory/project_authors_v030.md` を参照。
...
- MCP **サーバー**実装（v0.2.0 から繰り越し — Obsidian 等から LumenCite を参照可能に）
