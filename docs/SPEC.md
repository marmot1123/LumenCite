...
- **返すもの**: `focus` / `section_path` / `before` / `continuation`（読み順で次の構造境界の手前まで＝ページ境界で切れない）/ `proofs` / `proves` / `premises` / `equations` / `figures` / `citations` / `references` / `notes`。全要素に `origin` + `confidence`、PDF 版は `page` + `bbox`（既存ツールと同じ 4 要素配列）。
