...
- **OpenAI ChatGPT / Codex 対応（v0.5.0 候補）**: MCP サーバーはプロトコル汎用（JSON-RPC 2.0 / localhost HTTP + Bearer / stdio shim / トランスポート非依存のツールレジストリ）に実装済みのため、Claude 以外のクライアントへも拡張しやすい。**Codex（OpenAI CLI）**は既存 `--mcp-stdio` shim をほぼ流用できる見込みで、最小の第一歩は `get_mcp_server_config_snippet` への `"codex"` arm 追加＋設定 UI スニペット（Phase 3 の Claude Desktop 対応がテンプレ）と実機疎通検証。transport（stdio / streamable HTTP）と設定フォーマットは着手時に要確認。**ChatGPT connector** は公開到達可能なリモート + OAuth を要求しがちで localhost + Bearer では繋がらない可能性が高く、別スコープ（要件調査を先行）。
