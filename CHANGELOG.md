...
- **A backup no longer fails — or quietly loses files — because something was deleted while it was being written** — a full backup snapshots the database first and then spends minutes packing `attachments/` (7–9 minutes in a real library), so a file that disappears in between is a normal race, not a defect. Until now the outcome depended on exactly when it vanished: a file already gone when the packer looked at it was skipped without a word, while one that vanished a moment later failed the whole backup and threw the partial archive away. Both are now handled the same way — the entry is skipped, the backup completes, and every skipped path is listed in a `SKIPPED.txt` inside the archive as well as in the log, so **"nothing was lost" and "something was quietly lost" no longer look alike**. Restoring ignores that file. Anything other than a vanished entry — out of disk space, a permission error — still fails the backup rather than producing an archive with holes in it.

- **Rebuilding a paper's structure no longer collects its figure crops out from under a running backup** — building LCIR for a paper sends the previous version's crops to the trash. Doing that while a backup is packing left the archive holding database rows that point at images the archive does not contain; restoring from it re-renders those crops, and a re-rendered crop can lose the figure description that was paid for. The collection now stands aside while a backup is running and picks those directories up on the next build. Two things this does **not** cover: deleting an attachment or an entry, and reclaiming storage from Settings → Data, still run without regard for a backup in progress — but a backup that loses files that way now says so.
