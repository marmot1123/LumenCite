...
/// 抽出器のバグと区別がつかない。
///
/// **ファイル 1 枚ごとに tx を張る**（呼び出し側でまとめて張らない）。heal は
/// 版の全ファイルを描き直すので、att37（527 頁）なら 75 分ぶん書込ロックを
/// 保持することになるため。守りたい不変量は「`assets.sha256` と
/// `node_alt_texts.source_asset_sha256` が乖離しない」ことで、それは 1 枚単位で足りる。
