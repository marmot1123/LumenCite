...
/// 行を削除する。crop PNG 自体は 8a の GC で trash 済なので、行だけ残しても履歴価値が薄く
/// 肥大化するだけ。**`user_edited` は削除しない**（手編集の保護）。削除行数を返す。
        ...
        "DELETE FROM node_alt_texts
         WHERE origin = 'llm_inference'
           ...
           AND document_version_id IN (
               SELECT id FROM document_versions WHERE attachment_id = ?
           )",
    )
...
/// alt text 行がまだ無いもの。既に行があるノードは返さない（`user_edited` も含めて尊重 =
/// 手編集を Vision で上書きしない・再実行で再課金しない）。1 ノードに複数 `page_crop` が
/// ある場合は最初の 1 枚だけを対象にする。
