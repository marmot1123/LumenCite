...
fn tool_specs(write_on: bool) -> Vec<Value> {
    ...
    tools.push(json!({
        ...
        "description": "Return the detected figures (LCIR) of a paper — by entry_id or \
            ...
            alt_text — a description of the image itself, GENERATED by a vision model \
            (origin llm_inference) rather than taken from the paper. Treat it as a hint, never as \
            the authors' words, and prefer the caption when they disagree; it is absent unless the \
            user ran the opt-in generation batch in the app. Returns {has_lcir, \
...
async fn exec_get_figures(pool: &SqlitePool, args: &Value) -> Result<String, ToolError> {
    ...
    Ok(serde_json::to_string(&json!({
        ...
        "note": "figure regions come from embedded raster images (origin layout_model); vector \
            ...
            present, is generated by a vision model (origin llm_inference) — not the authors' \
            wording; the caption is the source of truth.",
    }))
    .unwrap_or_default())
}
