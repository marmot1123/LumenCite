...
//! 2. **大きい版の削除は 1 tx で 5.7〜7.95 秒かかる**（実測・最大は 272,583 ノードの版。
//!    下記 3 の索引を張る前は 16.7 秒だったが、**その索引は今この関数の入口で毎回保証する**
//!    ので、現行コードで測れるのは索引ありの値）。プールの `busy_timeout` は 30 秒
//!    （v1.0.0-p2 で明示・それ以前は sqlx 既定の 5 秒で、当時はここが `SQLITE_BUSY` の
//!    直接原因だった）なので今は落ちはしないが、**その秒数ぶんユーザーの保存が固まる**。
//!    **ノード数でチャンクを切る**。
...
/// `busy_timeout`（v1.0.0-p2 で 30 秒を明示。それ以前は sqlx 既定の 5 秒で、
/// この値を決めたときの根拠は 5 秒側だった）に対して 20 倍以上の余裕がある。
/// **版単位に割るだけでは足りない**（最大の版は 272,583 ノードで 5.7 秒）。
/// ⚠ 余裕が増えたからといってチャンクを上げてよい話ではない ── 待たせないための値であって、
/// `SQLITE_BUSY` を避けるためだけの値ではない（1 チャンクぶんは他の書き手が待たされる）。
...
mod tests {
    ...
    ///
    /// これが `NODE_DELETE_CHUNK` の存在理由（1 tx を短く保ち、他の書き手を待たせない）で、
    ...
    fn the_node_delete_chunk_is_fifty_thousand() {
        assert_eq!(
            ...
            "1 tx を短く保ち他の書き手を待たせないための値（busy_timeout は v1.0.0-p2 で 30 秒）。実測で 1 チャンク 0.28〜0.63 秒"
        );
    }
